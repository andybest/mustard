...
void PageAllocator::initialize() {
    ...
    for (uint32_t i = 0; i < pages4mb; i++) {
        ...
        uint32_t pdEntry = (physicalAddress & 0xFFC00000) | kPageDirectoryFlagPresent | kPageDirectoryFlagReadWrite |
                           kPageDirectoryFlagPageSize;
...
void PageAllocator::initialize_kernel_pagetables(const uint32_t kernelPages, const uint32_t pageTablesNeeded,
    ...
    for (uint32_t i = 0; i < pageTablesNeeded; i++) {
        ...
        uint32_t pageDirectoryEntry =
                (ptPhysicalAddress & 0xFFC00000) | kPageDirectoryFlagPresent | kPageDirectoryFlagReadWrite;
